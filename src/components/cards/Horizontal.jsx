...
const Horizontal = ({postData}) => {
  return (
    <Link to={`/post/${postData?.slug?.current}`} className='grid grid-cols-5 gap-5 grid-flow-col h-max mb-5 group cursor-pointer relative'>
            
            <div className="image col-start-1 col-end-3 relative overflow-hidden">
                {postData?.mainImage && <img loading='lazy' src={urlFor(postData?.mainImage).width(400).url()} alt="" className='group-hover:scale-110 transition-all duration-1000 ease  h-[150px] w-full object-cover'/>}
            ...
            <div className="col-start-3 col-end-6 flex flex-col gap-2 justify-between">
                    <div className="title">
                        <h3 className='text-md lg:text-lg font-bold'>{postData?.title}</h3>
                    </div>
                    <div className="info text-gray-400 items-start gap-2 flex flex-col">
                        ...
                        <small className='hidden sm:flex items-center gap-1 '>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                        </svg>
                            125 Comments</small>
                    </div>
                    {/* <div className="dsp text-sm sm:block hidden">
