...
const List = () => {
  ...
  return (
    <div className="p-5 bg-white my-4 border-b-4 border-yellow-500">
        ...
        {cats?.map(cat=>{
          return <div key={cat?._id} className="tag py-2 uppercase  px-2 inline-block bg-gray-200 text-xs text-gray-600 border-l-4 border-yellow-500 cursor-pointer hover:bg-yellow-500 hover:text-white transition-all duration-300 ease">{cat?.title}</div>
        })}
        </div>
    </div>
  )
